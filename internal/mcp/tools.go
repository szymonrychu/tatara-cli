...
func OperatorTools() []Tool {
	...
	return []Tool{
		...
		op("change_summary", "Post a change summary for the current task: PR title, PR body, delivered scope, change significance (semver level), and optionally what was most problematic. Implement the FULL agreed scope in this PR or call decline_implementation instead - do not leave scope remaining.",
			`{"type":"object","properties":{"task":{"type":"string"},"pr_title":{"type":"string"},"pr_body":{"type":"string"},"delivered_scope":{"type":"string"},"change_significance":{"type":"string","enum":["major","minor","patch"],"description":"major=backward-incompatible (API/CRD break, removed/renamed public surface); minor=backward-compatible new functionality; patch=fix or anything else"},"remaining_scope":{"type":"string","description":"MUST be left empty. A non-empty value means the implementation is INCOMPLETE and the task will FAIL (reason IncompleteImplementation) instead of proceeding to review/merge - no separate issue is filed. If part of the agreed scope cannot be delivered this turn, call decline_implementation instead of opening a partial PR."},"most_problematic":{"type":"string","description":"What was most problematic or surprising during implementation (gotchas, dead-ends, tricky integration points). Surfaced in the MR body and recorded in docs."}},"required":["pr_title","pr_body","delivered_scope","change_significance"]}`,
			func(a map[string]any) (string, string, any, error) {
				tk := argOrEnv(a, "task", "TATARA_TASK")
				if tk == "" {
					return "", "", nil, fmt.Errorf("task required")
