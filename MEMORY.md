...
2026-06-12 - **Chat MCP tools (issue #24).** Added a 10-tool `chat` group targeting tatara-chat REST: `chat_create_room`/`chat_list_rooms`(?status)/`chat_get_room`/`chat_close_room`(archive=DELETE /rooms/{id}); `chat_add_participant`/`chat_list_participants`/`chat_remove_participant`; `chat_send_message`(participant_id,body,kind?,target?)/`chat_poll_messages`(GET /rooms/{id}/messages?participant=, advances the per-participant cursor, response carries room_status)/`chat_get_log`. `Tool.Target` gained `TargetChat`; `Server` now holds three clients (memory+operator+chat) and `clientFor` switches on Target. Chat base URL resolves flag(--chat-base-url) > env(TATARA_CHAT_URL) > file(chatBaseUrl yaml) > default https://tatara.szymonrichert.pl/api/v1/chat. Same single OIDC token reused across all three targets (chat audience tatara-chat assumed carried, exactly like operator's 2026-06-06 entry); revisit if per-audience token exchange is ever needed. Room/participant ids are server-issued UUIDs passed back verbatim via url.PathEscape.
