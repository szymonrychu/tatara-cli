...
2026-06-09 - **tools/list serialization bug fixed.** `register()` built tools with `mcplib.NewTool(... WithRawInputSchema)`, but NewTool seeds a default object `InputSchema`; mcp-go v0.54.1 `Tool.MarshalJSON` errors when both InputSchema and RawInputSchema are set ("tool bulk_create_memories has both ..."). This aborted the entire tools/list marshal at the first tool alphabetically (bulk_create_memories), so the MCP server exposed ZERO tools to clients. Fix: extracted `buildTool()` which clears `tool.InputSchema = ToolInputSchema{}` after NewTool, leaving only the raw schema. Added TestBuildTool_Marshals regression test that json.Marshal's every tool (prior tests only checked registration didn't panic, never marshaling).
