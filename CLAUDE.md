# CLAUDE.md - tatara-cli

## What this repo is

The `tatara` CLI and the stdio MCP server every agent pod talks to: OIDC device
flow, REST passthrough to the platform services, and the tool surface itself.
The tool manifest published from here is what `tatara-agent-skills` validates
its documented calls against.

## What this repo is NOT

- Not a service. It is a binary distributed by brew tap and baked into the
  wrapper image.
- Not a policy layer. What an agent may do is the operator's tool profile, not a
  client-side check.
...
- Adding, renaming or removing an MCP tool breaks skills that document it.
  `tatara-agent-skills/.github/scripts/validate_tool_calls.py` reads the
